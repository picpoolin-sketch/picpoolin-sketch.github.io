...
let favorites = new Set(JSON.parse(localStorage.getItem("worldtv:fav") || "[]"));
let selected = null, mode = "all", hls = null;
...
// Verify stream endpoint using a HEAD request
...
function appendNextPage() {
  ...
  pageItems.forEach(c => f.appendChild(createChannelCard(c)));
...
function render() {
  ...
  filteredChannels = channels.filter(c => {
    const hasStreams = (streams.get(c.id) || []).length > 0;
    if (mode === "favorites" && !favorites.has(c.id)) return false;
    if (mode === "available" && !hasStreams) return false;
...
async function play(id) {
  ...
  const ss = streams.get(id) || [];
  el.now.textContent = c?.name || "Channel";
  el.meta.textContent = ss.length ? `${ss.length} stream${ss.length > 1 ? "s" : ""} listed` : "Checking availability...";

  if (!ss.length) {
    fail("No stream URL is listed for this channel.");
    return;
  }

  // Pre-validate streams to prevent playback errors
  let playableStream = null;
  for (const stream of ss) {
    const works = await isStreamReachable(stream.url);
    if (works) {
  ...
  if (!playableStream) {
    fail("Stream is currently offline, restricted, or unavailable.");
    return;
  }

  try {
    if (Hls.isSupported() && /\.m3u8($|\?)/i.test(playableStream.url)) {
      hls = new Hls({ enableWorker: true });
      hls.loadSource(playableStream.url);
      hls.attachMedia(el.video);
      hls.on(Hls.Events.MANIFEST_PARSED, () => el.video.play().catch(() => {}));
      hls.on(Hls.Events.ERROR, (_, d) => {
        if (d.fatal) fail("This stream could not be played. It may be offline or restricted.");
      });
    } else {
      el.video.src = playableStream.url;
...
async function init() {
  try {
    const [c, s, l, co, ca] = await Promise.all([
    ...
    channels = c.filter(x => !x.closed);
    s.forEach(x => {
      if (x.channel && x.url) {
        if (!streams.has(x.channel)) streams.set(x.channel, []);
        streams.get(x.channel).push(x);
      }
    });
    l.forEach(x => {
      if (x.channel) {
        if (!logos.has(x.channel)) logos.set(x.channel, []);
        logos.get(x.channel).push(x);
      }
    });
    co.forEach(x => countries.set(x.code, x));
    ca.forEach(x => cats.set(x.id, x));
    populate();
    render();
  } catch (e) {
    el.grid.innerHTML = '<div class="loading">Unable to load IPTV data. Check your connection and refresh.</div>';
  }
}
...
el.favChip.onclick = () => {
  ...
  el.availChip.classList.remove("active");
  render();
};
